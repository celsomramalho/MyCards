...
function adjustNumber(targetId, delta) { const input = document.getElementById(targetId); if (!input || input.disabled) return; const min = Number.isFinite(Number(input.min)) ? Number(input.min) : 0; const max = input.max === "" ? Infinity : Number(input.max); const value = Math.min(max, Math.max(min, (Number(input.value) || 0) + delta)); input.value = String(value); input.dispatchEvent(new Event("input", { bubbles: true })); }
